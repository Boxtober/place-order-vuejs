...
<body>
    <div id="app">
        <div class="container">
            <div class="row">
                <p class="jumbotron">Login : <input type="text" v-model="user"/> | {{ user }} est connecté</p>
            </div>

            <h1 class="display-6" v-if="commandes.length > 0">Produits commandés</h1>
    ...
    <script>
        Vue.component('fiche-produit',{
            template: `
            <div class="col-3">
                <p class="jumbotron"> {{ nom }} <br> <button v-on:click="passer_commande(nom)" v-if="role == 'commande'">Commander</button> </p>
            </div>
            `,
            props: ['nom', 'role'],
            methods:{
        ...
        var app = new Vue({
            ...
            data: {
                user: "sabrina",
                produits: ["Produit 1", "Produit 2", "Produit 3", "Produit 4"],
                commandes : [],
                affichage: "display-3"
            },
            methods:{
